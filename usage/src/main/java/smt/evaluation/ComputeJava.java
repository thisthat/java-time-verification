...
public class ComputeJava {
    ...
    public ComputeJava(String name, String path, String jars, String user) throws IOException {
        ...
        while (i.hasNext()) {
            ...
            //    continue;
            debug.log("[" + java_calls[0] + "] Processing " + filename);
            Java2AST a = null;
            try {
            ...
            for(ASTClass c : result){
                for(IASTMethod m : c.getMethods()) {
                    if(m.getAccessModifier() != IASTMethod.AccessModifier.PUBLIC)
                        continue;
                    final boolean[] visitedMethod = {false};
                    m.visit(new DefaultASTVisitor() {
                        @Override
                        public void enterASTMethodCall(ASTMethodCall elm) {
                            if(visitedMethod[0]) return;
                            for (TimeTypes u : java_api) {
                                if (u.isMethodCall(elm)) {
                                    java_calls[0]++;
                                    visitedMethod[0] = true;
                                    return;
                                }
                            }
                        }
    ...
    public static void main(String[] args) throws IOException {
        if(args.length < 4) {
            System.out.println("Usage with: project_name project_path lib_path user_types");
            System.exit(1);
        }
        String path = args[0];
        String name = args[1];
        ...
        String user = args[3];
        try {
            new ComputeJava(path, name, jars, user);
        } catch (Exception e){
